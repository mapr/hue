...
class HiveServer2Dbms(object):
  ...
  def describe_partition(self, db_name, table_name, partition_id):
    ...
    parts = [table.partition_keys[idx].name + "='%s'" % key for idx, key in enumerate(partitions[partition_id].values)]
    partition_spec = ','.join(parts)

    return self.client.describe_partition(db_name, table_name, partition_spec)
