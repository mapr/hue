...
class TestMetastoreWithHadoop(BeeswaxSampleProvider):
  ...
  def test_browse_partition(self):
    ...
    filebrowser_path = urllib.unquote(reverse("filebrowser.views.view", kwargs={'path': '/user/hive/warehouse/test_partitions/baz=baz_one/boom=boom_two'}))
    if is_live_cluster():
      filebrowser_path = urllib.unquote(reverse("filebrowser.views.view", kwargs={'path': '/user/hive/warehouse/%s.db/test_partitions/baz=baz_one/boom=boom_two' % self.db_name}))
