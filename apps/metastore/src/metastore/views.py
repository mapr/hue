...
def describe_table(request, database, table):
  ...
  cluster = json.loads(request.POST.get('cluster', request.GET.get('cluster', '{}')))
  source_type = request.POST.get('source_type', request.GET.get('source_type', 'hive'))
  db = _get_db(user=request.user, source_type=source_type, cluster=cluster)
  ...
  if request.POST.get("format", request.GET.get("format", "html")) == "json":
    ...
    partitions = None
    if app_name not in ('impala', 'drill', ):
      try:
    ...
    return render(renderable, request, {
      ...
      # 'table': table_obj,  # The fun fact is that this variable is never used in the template code.
