        ...
        if (vm.job() == self && self.apiStatus() == 'RUNNING') {
          lastFetchJobRequest = self._fetchJob(function (data) {
            if (['schedule', 'workflow'].indexOf(vm.job().type()) >= 0) {
              ...
              var coordinatorActions = vm.job().coordinatorActions();
              if (coordinatorActions) {
                var selectedIDs = coordinatorActions.selectedJobs().map(
              ...
              coordinatorActions = vm.job().coordinatorActions();
              if (coordinatorActions) {
                coordinatorActions.selectedJobs(
                  coordinatorActions.apps().filter(function(coordinatorAction){
