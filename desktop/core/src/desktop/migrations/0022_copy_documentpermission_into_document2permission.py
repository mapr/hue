...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for permission in orm['desktop.DocumentPermission'].objects.filter(doc__content_type=document2_contenttype):
            ...
            perm, created = orm['desktop.Document2Permission'].objects.get_or_create(doc=doc2, perms=permission.perms)
            perm.users = [ user.pk for user in permission.users.all() ]
            perm.groups = [ group.pk for group in permission.groups.all() ]
            perm.save()
    ...
    models = {
        ...
        u'desktop.document2': {
            ...
            'uuid': ('django.db.models.fields.CharField', [], {'default': "'ab2e5ed6-0ca8-41b2-a138-4531396bb975'", 'max_length': '36', 'db_index': 'True'}),
