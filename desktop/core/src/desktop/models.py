...
class ClusterConfig():
  ...
  def _get_browser(self):
    ...
    if 'filebrowser' in self.apps and ANALYTIC_DB not in self.cluster_type and fsmanager.is_enabled('hdfs') and fsmanager.has_access('hdfs', self.user):
      interpreters.append({
        'type': 'hdfs',
        'displayName': _('Files'),
        'buttonName': _('Browse'),
    ...
    if 'filebrowser' in self.apps and not IS_EMBEDDED.get() and fsmanager.is_enabled('s3a') and fsmanager.has_access('s3a', self.user):
      interpreters.append({
        'type': 's3',
        'displayName': _('S3'),
        'buttonName': _('Browse'),
    ...
    if 'filebrowser' in self.apps and ANALYTIC_DB not in self.cluster_type and fsmanager.is_enabled('adl') and fsmanager.has_access('adl', self.user):
      interpreters.append({
        'type': 'adls',
        'displayName': _('ADLS'),
        'buttonName': _('Browse'),
