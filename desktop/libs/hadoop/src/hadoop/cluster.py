...
def call_maprcli_urls(servicename):
  try:
    maprcli_popen = subprocess.Popen(["maprcli", "urls", "-name", servicename], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
...
YARNCLUSTER_MAPRCLI_CACHE = None
...
def get_yarncluster_from_maprcli(current_user=None):
  ...
  try:
    ...
    if not YARNCLUSTER_MAPRCLI_CACHE:
      LOG.info('get_yarncluster_from_maprcli(): Running maprcli to update config with right RM and HS')
    ...
    YARNCLUSTER_MAPRCLI_CACHE = (config, rm)
    ...
    return YARNCLUSTER_MAPRCLI_CACHE
  finally:
