...
class TestAWS(unittest.TestCase):
  def test_with_credentials(self):
    try:
      ...
      with patch('aws.client.conf_idbroker.get_conf') as get_conf:
        with patch('aws.client.Client.get_s3_connection') as get_s3_connection:
  ...
  def test_with_idbroker(self):
    try:
      ...
      with patch('hadoop.core_site.get_conf') as get_conf:
        with patch('aws.client.Client.get_s3_connection') as get_s3_connection:
          with patch('aws.client.IDBroker.get_cab') as get_cab:
            #with patch('aws.current_seconds_from_utc') as current_seconds_from_utc:
            get_conf.return_value = {
