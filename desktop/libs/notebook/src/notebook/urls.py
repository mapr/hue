...
urlpatterns += patterns('notebook.api',
  ...
  url(r'^api/autocomplete/(?P<database>[\w.]+)/(?P<table>[\w_\-.]+)/(?P<column>[\w_\-.]+)/?$', 'autocomplete', name='api_autocomplete_column'),
  url(r'^api/autocomplete/(?P<database>[\w.]+)/(?P<table>[\w_\-.]+)/(?P<column>[\w_\-.]+)/(?P<nested>.+)/?$', 'autocomplete', name='api_autocomplete_nested'),
