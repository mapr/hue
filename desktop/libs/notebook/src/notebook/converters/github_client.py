...
class GithubClient(object):
  ...
  def get_file_contents(self, username, repo, filepath):
    ...
    try:
      ...
      master = repo.get_git_tree('master', recursive=True)
      file = next(elem for elem in master.tree if elem.path == filepath)
      if not file and master.raw_data['truncated']:
        # TODO: Need to recursively call trees API
        raise NotImplementedError('Git tree exceeded maximum elements.')
      else:
        content = repo.get_git_blob(file.sha).content.decode('base64')
