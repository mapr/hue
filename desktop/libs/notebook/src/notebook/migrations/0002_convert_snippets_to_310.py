...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        import json
        for notebook in orm['desktop.Document2'].objects.filter(type='notebook'):
            data_python = json.loads(notebook.data)
            for snippet in data_python['snippets']:
                snippet['type'] = snippet_type_map.get(snippet['type'], snippet['type'])
            for session in data_python['sessions']:
                session['type'] = snippet_type_map.get(session['type'], session['type'])
            notebook.data = json.dumps(data_python)
            notebook.save()

    def backwards(self, orm):
        raise RuntimeError('Cannot reverse this migration / not implemented.')
    ...
    models = {
        ...
        u'desktop.document2': {
            ...
            'uuid': ('django.db.models.fields.CharField', [], {'default': "'99dbb312-5122-42fa-8af8-4b2bbd82d5a0'", 'max_length': '36', 'db_index': 'True'}),
    ...
    complete_apps = ['desktop', 'notebook']
    symmetrical = False
